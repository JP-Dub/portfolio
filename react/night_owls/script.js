...
import "../../public/css/night_owls.css";
...
class Main extends Component {
  render() {
    return (
      <BrowserRouter>
        <Route exact path="/react/night_owls" strict component={App} /> 
        <Route exact path="/react/night_owls/rsvp/demo" component={App} />
        <Route exact path="/react/night_owls/login/:user" component={App} />
      </BrowserRouter>
    );
  }
}
...
class App extends Component {
  ...
  componentDidMount() {
    ...
    this.setState(state => {
      return {
        title: (
          state.title = RegExp("^/react/night_owls/rsvp/.*").test(path) 
          ? "Night Owls Demo"
          : "Night Owls"
        )
      };
    ...
    this.login = RegExp("^/night_owls/login/.*").test(path);
    this.demo  = RegExp("^/react/night_owls/rsvp/.*").test(path);
    ...
    this.findLocation = (evt) => {
      ...
      if (location.match(/demo/i)) return window.location.href = "/react/night_owls/rsvp/demo";
      ...
      params =
        typeof data === "string"
          ...
          : Object.keys(data)
              ...
              .join("&");
    console.log('/night_owls/scripts.js 481 - url', url)
